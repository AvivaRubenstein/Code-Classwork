...
function App() {
      //useState stores the state of something in our app
  // We declare a state variable that is an array called `issues` and a function to update it.
  const [issues, setIssues] = useState([]);

  // When the page loads, set the document title to something specific to this app. Runs once because of optional dependency array -- empty brackets
      //we could also decide that this useEffect should happen when a specific state change happens, we would put that state in the brackets
      //by default, useEffect will trigger when there is a state change/ re-rendering of our app
  useEffect(() => {
